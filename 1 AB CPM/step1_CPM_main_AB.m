...
% Bahavior data
load('behav_vec_AB.mat');

    % 去除异常的行为被试, 包括 行为数据和FC数据
    ...
    save(['AB_pos_mat_NC001'], 'pos_mat_save');  
    save(['AB_neg_mat_NC001'], 'neg_mat_save');  
   ...
   %% plot of positive network prediction
    ...
    csvwrite('AB_Postive_network_prediction.csv',Postive_network_prediction);
